...
TMap<FString, FPolygon> LMParkingLot::Create(FVector size, FModelParams modelParams,
    ...
    float parkingStripDepth = 6;
    float treeStripDepth = 1;
    int rowCount = floor(size.Y / (parkingStripDepth + treeStripDepth));
    UE_LOG(LogTemp, Warning, TEXT("LMParkingLot.Create rowCount %d"), rowCount);
    float treeGap = 15;
    int treeCountInRow = floor(size.X / treeGap);
    float centerY = - (size.Y / 2 - parkingStripDepth / 2 - treeStripDepth / 2);
    createParamsIn.offset += FVector(0, centerY, 0);

    for (int ii = 0; ii < rowCount; ii++){
        FVector stripSize = FVector((size.X, parkingStripDepth, size.Z));
        LMParkingLot::CreateStrip(stripSize, modelParams, createParamsIn); 

        for (int jj = 0; jj < treeCountInRow; jj++){
            uName = Lodash::GetInstanceId("pineS");
            pairsString = "mesh=pineS";
            vertices = {MathVector::RotateVector(FVector((jj - 1) * treeGap + treeGap / 2 , -6, 0), createParamsIn.rotation) + createParamsIn.offset};
        ...
        centerY += parkingStripDepth + treeStripDepth;
    }
...
TMap<FString, FPolygon> LMParkingLot::CreateStrip(FVector size, FModelParams modelParams,
                                             FModelCreateParams createParamsIn)
{
    FString uName, type, pairsString, scaleString;
    TArray<FVector> vertices;
    ...
    UE_LOG(LogTemp, Warning, TEXT("LMParkingLot.CreateStrip scaleString %s"), *scaleString);
    ...
    for (int ii = 0; ii < solarChargingStationCount; ii++){
        ...
        vertices = {MathVector::RotateVector(FVector(slot1X, -(size.Y / 2 - evChargerDepth), 0), createParamsIn.rotation) + createParamsIn.offset};
        polygons.Add(uName, FPolygon(uName, uName, vertices, FVector(0, 0, 0), "evCharger", "point", pairsString));

        uName = Lodash::GetInstanceId("EVCharger");
        pairsString = "mesh=evCharger";
        vertices = {MathVector::RotateVector(FVector(slot2X, -(size.Y / 2 - evChargerDepth), 0), createParamsIn.rotation) + createParamsIn.offset};
        polygons.Add(uName, FPolygon(uName, uName, vertices, FVector(0, 0, 0), "evCharger", "point", pairsString));
        ...
        if (slot1CarProb > 0.3){
            ...
            vertices = {MathVector::RotateVector(FVector(slot1X - 2, 3, 0), createParamsIn.rotation) + createParamsIn.offset}; // note: this car mesh seems not centered in the model
        ...
        if (slot2CarProb > 0.3){
            uName = Lodash::GetInstanceId("Car");
            pairsString = "mesh=mercedes";
            vertices = {MathVector::RotateVector(FVector(slot2X - 2, 3, 0), createParamsIn.rotation) + createParamsIn.offset};
            polygons.Add(uName, FPolygon(uName, uName, vertices, FVector(0, 0, 0), "car", "point", pairsString));
        }
