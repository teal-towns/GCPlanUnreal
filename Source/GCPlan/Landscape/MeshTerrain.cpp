...
void MeshTerrain::DestroyRoads() {
	_RoadsVertices.Empty();
}

void MeshTerrain::AddRoads(TMap<FString, TArray<FVector>> roadsVertices) {
	for (auto& Elem : roadsVertices) {
		FString UName = Elem.Key;
		if (!_RoadsVertices.Contains(UName)) {
			_RoadsVertices.Add(UName, Elem.Value);
		}
	}
	DrawRoads();
}

void MeshTerrain::DrawRoads(float widthMeters, float stepFactor) {
	HeightMap* heightMap = HeightMap::GetInstance();
	auto [XSize, YSize, MapXSize] = heightMap->GetImageSize();
	float MetersPerPixel = (float)MapXSize / (float)XSize;
	float stepDistance = widthMeters * stepFactor;
	int pixelRange = round(widthMeters / MetersPerPixel);
	int pixelRangeHalf = round(pixelRange / 2);
	...
	for (int yy = 0; yy < YSize; yy++) {
		for (int xx = 0; xx < XSize; xx++) {
			// if (yy % 2 == 0) {
			// 	pixelData.Add(FColor(255,255,255,0));
			// } else {
	...
	int splatsCount = 0;
	for (auto& Elem : _RoadsVertices) {
		FString UName = Elem.Key;
		TArray<FVector> vertices = Elem.Value;
